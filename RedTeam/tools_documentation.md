...
*   **Script Scanning (`-sC` or `--script`):** Runs default Nmap scripts or specified scripts to perform more advanced detection, vulnerability scanning, and exploitation.
    ...
    ```
*   **Firewall Evasion (`-f`, `--mtu`, `--data-length`, `--badsum`):** Various options to try and bypass firewalls.
    ```bash
    nmap -f target.com # Fragment packets
    ```
*   **Output Formats (`-oN`, `-oX`, `-oG`, `-oA`):** Save scan results in normal, XML, Grepable, or all formats.
    ```bash
...
NSE is one of Nmap's most powerful and flexible features. It allows users to write (and share) simple scripts to automate a wide variety of networking tasks. These scripts are executed in parallel with the speed and efficiency you expect from Nmap.
...
This section will be expanded with more tools like Metasploit, Mimikatz, and various C2 frameworks.
